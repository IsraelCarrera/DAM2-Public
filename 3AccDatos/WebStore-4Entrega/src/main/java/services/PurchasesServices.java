...
public class PurchasesServices {
    ...
    public List<Purchase> getPurchaseByIdItem(int idItem) {
    ...
    public List<Purchase> getPurchasesBetweenDates(LocalDate iniDate, LocalDate finDate){
        return dao.getDAOPurchases().getPurchasesBetweenDates(iniDate, finDate);
    }

    public double getPurchasesInMonth(int idItem){
        return dao.getDAOPurchases().countPurchasesInMonth(idItem);
    }
}

...
public class FXMLAddReviewController implements Initializable {
    ...
    private void loadPurchases() {
        ...
        purchaseBox.getItems().clear();
        List<Purchase> list = ps.getPurchasesByClientId(clientBox.getSelectionModel().getSelectedItem().getDniCustomer());
        if(list !=  null) {
        ...
        try {
            if (purchaseBox.getSelectionModel().getSelectedItem() != null && clientBox.getSelectionModel().getSelectedItem() != null) {
                ...
                if(!title.isEmpty() && !descr.isEmpty()) {
                    Review review = new Review(rating, title, descr, p.getIdPurchase(), p.getNameItem(),clientBox.getSelectionModel().getSelectedItem().getDniCustomer());
                    boolean isAdd = rs.addReview(review,p);
                    if (isAdd) {
    ...
    private void loadClient(String idUser){

...
public class FXMLAddPurchasesController implements Initializable {
    ...
    private void addPurchase() {
        ...
        if (!customerBox.getSelectionModel().isEmpty() && !itemBox.getSelectionModel().isEmpty()) {
            ...
            Purchase purchase = new Purchase(UUID.randomUUID().toString().replace("-", ""),
                    customerBox.getSelectionModel().getSelectedItem().getDniCustomer(),
                    itemBox.getSelectionModel().getSelectedItem().getName(),
                    datePurchase);
            boolean isDone = ps.addPurchase(purchase);
            if (isDone) {

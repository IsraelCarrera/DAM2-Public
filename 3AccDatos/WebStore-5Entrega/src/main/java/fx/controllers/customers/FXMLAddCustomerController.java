...
public class FXMLAddCustomerController implements Initializable {
    ...
    private void addCustomer() {
        ...
        try {
            ...
            User user = new User(dni,username, password,"user");
            Customer customer = new Customer(dni,name, phone, adress);
            String idCustomer = customer.getDniCustomer();
            boolean isAdd = cs.addCustomer(customer, user);
            if (isAdd) {
                alerta = new Alert(AlertType.INFORMATION);
                alerta.setContentText(Constantes.CUSTOMER_ADD_FINE);
                alerta.showAndWait();
                customer.setDniCustomer(idCustomer);

...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        botonLista = this.findViewById(R.id.btVerLista)
    ...
    private fun annadirPersona() {
        if (!estaEnLista(contador)) {
            ...
            if (nombre.isEmpty() || pass.isEmpty() || mail.isEmpty() || tel.isEmpty()) {
                Toast.makeText(this, "No se ha agregado a la persona indicada.", Toast.LENGTH_SHORT)
                    .show()
            } else {
                val persona =
                    Persona(contador, nombre, pass, tel, mail, sexo, suscr, juegaPc, juegaConsola)
                (listPersonas.add(contador, persona))
                Toast.makeText(
                    this,
                    "la persona llamada $nombre se ha registrado con éxito",
                    Toast.LENGTH_SHORT
                ).show()
                tvListaTotal.text = listPersonas.size.toString()
            }
    ...
    private fun deletePersona() {
        if (estaEnLista(contador)) {
            ...
            Toast.makeText(
                this,
                "No hay persona aquí. No puedes borrar a nadie que no existe.",
                Toast.LENGTH_SHORT
            ).show()
        }
    ...
    private fun updatePersona() {
        if (estaEnLista(contador)) {
            ...
            if (nombre.isEmpty() || pass.isEmpty() || mail.isEmpty() || tel.isEmpty()) {
                Toast.makeText(
                    this,
                    "No se ha modificado a la persona indicada.",
                    Toast.LENGTH_SHORT
                ).show()
            } else {
                val persona =
                ...
                Toast.makeText(
                    this,
                    "la persona llamada $nombre ha recibido la modificación que ha indicado",
                    Toast.LENGTH_SHORT
                ).show()
            }
        } else {
            Toast.makeText(
                this,
                "No hay persona guardada aquí. Primero, guarda una persona.",
                Toast.LENGTH_SHORT
            ).show()
        }
    }
    ...
    //ir a la 2 pantalla para verlo como una lista.
    private fun irRecyclerView() {
        val personas: ArrayList<Persona> = listPersonas as ArrayList<Persona>
        val intent = Intent(this, ReciclerActivity::class.java)
        intent.putExtra(getString(R.string.persona), personas)
        startActivity(intent)
    }

    //Funciones para "mantener" los datos
    override fun onSaveInstanceState(outState: Bundle) {
        super.onSaveInstanceState(outState)
        Log.i("MyTag", "onSaveInstanceState")

        outState.putParcelableArrayList("List_key", ArrayList(listPersonas))
    }
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        ...
        listPersonas = savedInstanceState.getParcelableArrayList("List_key")!!
    }
}

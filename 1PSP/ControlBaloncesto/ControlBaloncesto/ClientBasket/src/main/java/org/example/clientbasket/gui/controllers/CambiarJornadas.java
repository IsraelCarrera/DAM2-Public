...
public class CambiarJornadas implements Initializable {
    ...
    private void addJornada() {
        try {
            ...
            Single<Either<String, Jornada>> singleJornadas = Single.fromCallable(() -> sj.addJornada(jornada))
    ...
    private void updateJornada() {
        try {
            LocalDate fecha = dpFechaJornada.getValue();
            if (lvJornada.getSelectionModel().getSelectedItem() != null) {
                Single<Either<String, Jornada>> singleJornadas = Single.fromCallable(() -> sj.updateJornada(lvJornada.getSelectionModel().getSelectedItem(), fecha))
                        .subscribeOn(Schedulers.io())
                        .observeOn(JavaFxScheduler.platform())
                        .doFinally(() -> principal.getPrincipal().setCursor(Cursor.DEFAULT));
    ...
    private void deleteJornada() {
        if (lvJornada.getSelectionModel().getSelectedItem() != null) {
            Single<Either<String, Integer>> singleJornadas = Single.fromCallable(() -> sj.deleteJornada(lvJornada.getSelectionModel().getSelectedItem().getIdJornada()))
                    .subscribeOn(Schedulers.io())
                    .observeOn(JavaFxScheduler.platform())
                    .doFinally(() -> principal.getPrincipal().setCursor(Cursor.DEFAULT));

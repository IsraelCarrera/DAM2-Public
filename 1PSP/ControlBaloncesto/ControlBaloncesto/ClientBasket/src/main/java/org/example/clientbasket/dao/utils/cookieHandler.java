...
public class cookieHandler implements CookieJar {
    ...
    public cookieHandler(CookieHandler cookieHandler) {
    ...
    public void saveFromResponse(HttpUrl url, List<Cookie> cookies) {
        if (this.cookieHandler != null) {
            ...
            Map multimap = Collections.singletonMap(Constantes.SET_COOKIE, cookieStrings);

            try {
                this.cookieHandler.put(url.uri(), multimap);
            } catch (IOException var6) {
                Platform.get().log(5, Constantes.SAVING_COOKIES_FAILED_FOR + url.resolve(Constantes.BARRA_PUNTOS), var6);
            }
        }
    ...
    public List<Cookie> loadForRequest(HttpUrl url) {
        ...
        try {
            ...
            Platform.get().log(5, Constantes.LOADING_COOKIES_FAILED_FOR + url.resolve(Constantes.BARRA_PUNTOS), var10);

package org.example.encriptarasimetrico.security;

import com.google.common.primitives.Bytes;
import io.vavr.control.Either;
import lombok.extern.log4j.Log4j2;
import org.example.encriptarasimetrico.security.utils.Constantes;
...
public class EncriptarSimetrico {
    ...
    public Either<String, String> encriptando(String cosaAEncriptar, String passEncriptado) {
        ...
        try {
            ...
            resultado = Either.right(Base64.getUrlEncoder().encodeToString(Bytes.concat(iv, salt,
                    cipher.doFinal(cosaAEncriptar.getBytes(StandardCharsets.UTF_8)))));
        } catch (Exception e) {
            resultado = Either.left(Constantes.NO_SE_HA_PODIDO_ENCRIPTAR_EL_MENSAJE);
        }
        return resultado;
    }

    public Either<String, String> desencriptando(String cosaADesencriptar, String passDesencriptar) {
        Either<String, String> resultado;
        try {
            ...
            resultado = Either.right(new String(cipher.doFinal(Arrays.copyOfRange(decoded, Constantes.VENTIOCHO, decoded.length))));
        } catch (Exception e) {
            resultado = Either.left(Constantes.NO_SE_HA_PODIDO_DESENCRIPTAR_EL_MENSAJE);
        }
        return resultado;
    }
}
